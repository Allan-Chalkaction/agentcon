...
> **AC-013 §5 wording note (Architect Acknowledgement Round 6 — 2026-05-06; supersedes the disputed sub-clause).** The literal "and" in the surround-contrast sub-clause above is **out of sync** with the operative disjunctive contract established at the highest approval tier (§8.12 "(disjunction-binding — see ADR D14)"; ADR D14 final-resolution paragraph; §9 Phase 3 implementation note line 756; PM Round 2 sign-off §11 line 1497; CONS-13 in the consensus ledger). The disjunctive contract is the binding one for test design and Phase 5 assert-leg authoring. **The sub-clause governing button-vs-surround discernibility on the settings surface reads as a disjunction:** *the button is visually discernible from the settings outer chrome background (`--bg-base` or renamed equivalent) — assertable as either (a) `background-color` contrasts with the surround at ≥3.0:1, **OR** (b) `border-color` (distinct from `background-color`) contrasts with the surround at ≥3.0:1.* The first clause (color-vs-background ≥4.5:1) is unchanged and remains conjunctively required. The original AC-013 prose immediately above is preserved verbatim for audit; this note governs in case of conflict. Discovered Phase 3 QA (`docs/pipeline/2026-05-05/design-migration-cream-panels/qa/phase-03-attempt-1-verdict.md` §4); adjudicated Phase 3 Reviewer (`docs/pipeline/2026-05-05/design-migration-cream-panels/reviewer/phase-03-attempt-1-verdict.md` Item 2); landed Architect Acknowledgement Round 6 — 2026-05-06.
...
### Architect Acknowledgement (Round 6) — 2026-05-06 (Phase 3 AC-013 §5 wording correction)

Phase 3 Attempt 1 surfaced an AC-013 §5 verbatim wording inconsistency: the §5 prose at line 127 used a conjunctive "and" between the two surround-discernibility sub-clauses, while §8.12 ("disjunction-binding — see ADR D14"), ADR D14's final-resolution paragraph, §9 Phase 3 implementation note (line 756), PM Round 2 sign-off (§11 line 1497), and CONS-13 all establish the operative reading as a disjunction (a OR b). QA Phase 3 caught the literal-vs-architectural conflict during the AC-013 adjudication; Reviewer routed the discrepancy to Architect with pre-drafted addendum text rather than failing Phase 3; this round lands the §5 correction. No code, test, or other §1–§10 governance is altered.

- **AC-013 §5 wording note appended (addendum approach, not in-place edit).** A short note immediately follows AC-013 at §5 line 127 that explicitly supersedes the disputed sub-clause "and" with the disjunctive reading from §8.12 / ADR D14 / §9 Phase 3 / PM Round 2 / CONS-13. Addendum was chosen over in-place edit so the original AC-013 verbatim is preserved intact for audit (matches the pattern of Acknowledgements R3, R4, R5; reviewer's pre-drafted text accommodates either approach). The note quotes the locked disjunction verbatim — clause (a) `background-color` vs surround ≥3.0:1, OR clause (b) `border-color` vs surround ≥3.0:1 — and confirms the color-vs-background ≥4.5:1 first clause is unchanged.
- **Phase 3 implementation already correct; no code change required.** Builder's `tests/dev-switch.test.tsx` asserts the disjunction with clause (b) satisfied (cream border vs `--bg-base` ≈16.31:1 per Reviewer's third-witness recomputation; clause (a) intentionally non-compliant at ≈1.15:1, expected per ADR D14). The wording correction is governance-level only; Phase 3 Reviewer verdict is REVIEWER_PASS, no re-execution is triggered.
- **Phase 5 forward-compatibility.** A Phase 5 Builder reading AC-013 §5 verbatim during exploration will now encounter the disjunctive contract directly at the AC site (no two-hop chase to §8.12 → D14) and will not flag clause (a)'s 1.15:1 ratio as a failure when authoring assert legs that touch the dev-switch surface.
- **Cross-references.** Phase 3 QA verdict §4 (AC-013 adjudication); Phase 3 Reviewer verdict Item 2 (pre-drafted addendum text + third-witness contrast 14.20:1 / 16.31:1); ADR D14 final-resolution paragraph + "Until that clarification" paragraph; §8.12 disjunction-binding row; §9 Phase 3 implementation note line 756; PM Round 2 sign-off concern #2 (§11 line 1497); CONS-13 in consensus ledger.
- **Scope discipline.** §8.12, ADR D14, §9, PM Round 2, CTO Round 1/2, prior Architect Acknowledgements R2/R3/R4/R5, Final Consensus Lock, and CONS ledger entries are unchanged — they already say the right thing. Only AC-013 §5 itself was out of sync; only AC-013 §5 was edited.

No other changes; consensus not reopened. Pipeline continues to Phase 4.
